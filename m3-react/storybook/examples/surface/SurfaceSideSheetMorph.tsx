...
const SurfaceSideSheetMorph: FC = () => {
  ...
  const [modalWidth, setModalWidth, modalWidthRef] = useStateRef(sideSheetWidthRef.current)
  ...
  const [modalTransitionMs, setModalTransitionMs] = useStateRef(PANEL_TRANSITION_MS)
  ...
  const setModalGeometryFromDocked = (fallbackWidth: number) => {
    const docked = measureDockedGeometry()

    setModalWidth(docked?.width ?? fallbackWidth)
  ...
  const setModalGeometryTarget = (width: number) => {
    setModalWidth(width)
  ...
  const expandModalToViewportHeight = async (width: number) => {
    setModalTransitionMs(PANEL_VERTICAL_EXPAND_MS)
    await raf()
    setModalGeometryTarget(width)
    await wait(PANEL_VERTICAL_EXPAND_MS)
    setModalTransitionMs(PANEL_TRANSITION_MS)
  }

  const resolveDockedTargetGeometry = (width: number) => {
    const layout = layoutRoot.current?.getBoundingClientRect()
  ...
  const switchDockedToModal = async () => {
    const nextWidth = syncFixedWidth()
    setModalGeometryFromDocked(nextWidth)
    ...
    await expandModalToViewportHeight(nextWidth)
  }

  const switchModalToDocked = async () => {
    const nextWidth = syncFixedWidth()
    const dockedTarget = resolveDockedTargetGeometry(nextWidth)
